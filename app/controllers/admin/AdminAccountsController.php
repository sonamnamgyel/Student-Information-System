...
class AdminAccountsController extends AdminController {
    ...
    public function getIndex()
    {
        ...
        $data = null;
        // Show the page with list of all accounts
        return View::make('admin/accounts/index', compact('title','student','departments','programmes','data'));
    }
    ...
    public function getSearch()
    {
        ...
        $student = $this->student;
        $departments = Department::lists('department_name', 'id');
        $programmes = Programme::lists('programme_name', 'id');
    ...
    {
        // Title
        $title = "Student Advanced Searching";
        $student = $this->student;
        $programmes = Programme::lists('programme_name', 'id');
        $departments = Department::lists('department_name', 'id');
        $schools = DB::table('schools')->lists('name', 'id');
     ...
     */
    public function getData($dep=null, $prog=null, $type=null, $sem=null, $reg=null, $fee_paid=null)
    {
        $searchResults = Student::leftJoin('programmes','students.programme_id','=','programmes.id')
            ...
            ->select(array('students.id','students.stdno',DB::raw('CONCAT_WS(\' \',students.fname,students.mname, students.lname) as name'),
                'students.stdtype', 'semesters.roman', 'programmes.programme_code', 'fees.paid','students.registered', 'students.id as actions'))
            ->orderBy('students.programme_id')
            ->orderBy('students.current_semester','desc')
            ->orderBy('students.fname')
            ->where(function($query) use ($dep, $prog, $type, $sem, $reg, $fee_paid) {
                if($dep)
